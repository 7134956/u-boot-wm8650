...
#ifdef WMT_FTBLK_VGA
	DPRINT("VGA CSC %d, DVO CSC %d\n",vppif_reg32_read(GOVRH_VGA_YUV2RGB_ENABLE),vppif_reg32_read(GOVRH_DVO_YUV2RGB_ENABLE));
#endif
...
unsigned int govrh_set_clock(unsigned int pixel_clock)
{
	...
	clock = auto_pll_divisor(DEV_DVO,SET_PLLDIV,0,pixel_clock);
	if(  clock == -1 ){
		DPRINT("[GOVRH] *E* set clock\n");
	}
	DPRINT("[GOVRH] set clock %d ==> %d\n",pixel_clock,clock);
#else
	// manu_pll_divisor(DEV_DVO,52,5,2,2); // 65M
	if( pixel_clock > 60000000 ){
		vppif_reg32_out(0xd8130208,0x00005434);
		while ((*(volatile unsigned int *)0xd8130000)&0x18);
		while ((*(volatile unsigned int *)(0xd8130004))&0x1FF);
		vppif_reg8_out(0xd8130374,0x00001);
	}
	if( pixel_clock == 29500000 ){
		vppif_reg32_out(0xd8130208,0x00005476);
		while ((*(volatile unsigned int *)0xd8130000)&0x18);
		while ((*(volatile unsigned int *)(0xd8130004))&0x1FF);
		vppif_reg8_out(0xd8130374,0x00005);
	}
	else {
		vppif_reg32_out(0xd8130208,0x00000C03);
		while ((*(volatile unsigned int *)0xd8130000)&0x18);
		while ((*(volatile unsigned int *)(0xd8130004))&0x1FF);
		vppif_reg8_out(0xd8130374,0x00001);
	}
...
void govrh_CUR_set_color_key(int enable,int alpha,unsigned int colkey)
{
	...
	vppif_reg32_write(GOVRH_CUR_ALPHA_ENABLE,alpha);
	vppif_reg32_write(GOVRH_CUR_COLKEY_ENABLE,enable);
}
...
void govrh_CUR_set_framebuffer(vdo_framebuf_t *fb)
{
	...
	govrh_CUR_set_coordinate(0,0,fb->img_w-1,fb->img_h-1);
}
...
int govrh_irqproc_set_position(void *arg)
{
	...
	width = p_govrh->fb_p->fb.img_w - p_cursor->posx;
	width = (width < p_cursor->fb_p->fb.img_w)? width:p_cursor->fb_p->fb.img_w;
	vppif_reg32_write(GOVRH_CUR_WIDTH,width);
	govrh_CUR_set_position(p_cursor->posx,p_cursor->posy);
	return 0;		
}
...
void govrh_CUR_proc_view(int read,vdo_view_t *view)
{
	...
	if( read ){
		...
		// govrh_CUR_set_position(view->posx,view->posy);
		vpp_irqproc_work(VPP_INT_GOVRH_VBIS,(void *)govrh_irqproc_set_position,0,0);
	}
#endif
}
...
void govrh_suspend(int sts)
{
	switch( sts ){
		...
		case 2:	// backup register
			...
			govrh_vo_clock = vpp_get_base_clock(VPP_PLL_C,VPP_DIV_DVO);
			break;
		default:
			break;
	}
...
//	vppif_reg32_write(0xd8130250,BIT18,18,1);	// DVO clock enable
//	vppif_reg32_write(0xd8130250,BIT30,30,1);	// GOVRH clock enable
...
int govrh_mod_init(void)
{
	...
	mod_fb_p->capability = BIT(VDO_COL_FMT_YUV420) | BIT(VDO_COL_FMT_YUV422H) | BIT(VDO_COL_FMT_YUV444) | BIT(VDO_COL_FMT_ARGB)
							| VPP_FB_FLAG_CSC | VPP_FB_FLAG_MEDIA | VPP_FB_FLAG_FIELD | BIT(VDO_COL_FMT_RGB_1555) | BIT(VDO_COL_FMT_RGB_565);
	...
	if( resx % 64 )
		resx += (64 - (resx % 64));	
	fb_p->img_w = resx;
	fb_p->img_w = vfb_var.xres;
	fb_p->img_h = vfb_var.yres;
