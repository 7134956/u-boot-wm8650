...
*/
	...
	1, 0, 1, NAND_TYPE_MLC, 0x140A0F46 /* 0x140A0C46 */, "SAMSUNG_K9G8G08U0A"}, // tADL may too small
...
int test_nand_table(unsigned int chip)
{
	...
	count = *(volatile unsigned int *)PMNAND_ADDR;
	if ((count&0x1FF) != g_nfinfo[chip].wmt_nfc.divisor) {
		if (set_nand_div(g_nfinfo[chip].wmt_nfc.divisor))
			return -1;
	}
	count = 0;
