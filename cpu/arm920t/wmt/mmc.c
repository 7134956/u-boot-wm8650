...
int SD_Init(void)
{
	...
	if  (SDDevInfo->MMC_Card == SD_TRUE) {
		...
		if (SDDevInfo->MMCVersion >= MMC4_01) {
			...
			tmp = *(mmc_ext_csd + 196);
		  	if (tmp == 0x03) {	/* high speed and max clock rate at 52Mhz*/
...
{
	unsigned long sd_status;
	struct _SD_PDMA_DESC_S ReadDesc;//= (unsigned long *)(0x00D00000);
...
/****************************************************/
{
	...
	get_chip_version();
	if (chip_id == 0x3437 || chip_id == 0x3429 || chip_id == 0x3451 || chip_id == 0x3465) {
		if (device_num > 1)
			device_num = 1;
		if (device_num < 0)
			device_num = 0;
...
#define GPIO_SD1_CD				BIT5

#define	GPIO_SD0_Data			(BIT0 | BIT1 | BIT2 | BIT3)
#define	GPIO_SD0_Clock			BIT4
#define	GPIO_SD0_WriteProtect	BIT5
#define	GPIO_SD0_Command		BIT6
#define	GPIO_SD0_POWER			BIT7

#define	GPIO_SD1_Command		BIT1
#define	GPIO_SD1_Clock			BIT0
#define	GPIO_SD1_WriteProtect	BIT5
#define	GPIO_SD1_Data			(BIT0 | BIT1 | BIT2 | BIT3 | BIT4 | BIT5 | BIT6 | BIT7)
...
#define GPIO_SD1_PinShare		BIT1
		...
		//SD1_PIN = ( GPIO_SD1_Clock | GPIO_SD1_Command | GPIO_SD1_WriteProtect | GPIO_SD1_Data);
		...
		/*set SD power pin as GPO pin & internel pull up then disable power*/
		if (SDDevInfo->CtrlID == 0) {
			/*set SD0 power pin as GPO pin*/
			REG8_VAL(GPIO_BASE_ADDR + 0x4D) |= GPIO_SD0_POWER;
			REG8_VAL(GPIO_BASE_ADDR + 0x8D) |= GPIO_SD0_POWER;
			/*set internal pull up*/
			REG8_VAL(GPIO_BASE_ADDR + 0x4CD) |= GPIO_SD0_POWER;
			/*set internal pull enable*/
			REG8_VAL(GPIO_BASE_ADDR + 0x48D) |= GPIO_SD0_POWER;
			/*disable SD0 power*/
			REG8_VAL(GPIO_BASE_ADDR + 0xCD) |= GPIO_SD0_POWER;
			
		} else if (SDDevInfo->CtrlID == 1) {
			/*set SD1 power pin as GPO pin*/
			REG8_VAL(GPIO_BASE_ADDR + 0x5D) |= GPIO_SD1_POWER;
			REG8_VAL(GPIO_BASE_ADDR + 0x9D) |= GPIO_SD1_POWER;
			/*set internal pull up*/
			REG8_VAL(GPIO_BASE_ADDR + 0x4DD) |= GPIO_SD1_POWER;
			/*set internal pull enable*/
			REG8_VAL(GPIO_BASE_ADDR + 0x49D) |= GPIO_SD1_POWER;
			/*disable SD1 power*/
			REG8_VAL(GPIO_BASE_ADDR + 0xDD) |= GPIO_SD1_POWER;
		}
		...
		/*  Config SD PIN share  */ 
		if (SDDevInfo->CtrlID == 1) {	
			REG32_VAL(GPIO_BASE_ADDR + 0x200) |= GPIO_SD1_PinShare;
		}

		/*  Config SD to GPIO  */
		if (SDDevInfo->CtrlID == 0) {
			REG8_VAL(GPIO_BASE_ADDR + 0x4D) |= SD0_PIN;
			REG8_VAL(GPIO_BASE_ADDR + 0x43)	|= GPIO_SD0_CD;
		}else if (SDDevInfo->CtrlID == 1) {
			REG8_VAL(GPIO_BASE_ADDR + 0x4F) |= GPIO_SD1_Data;
			REG8_VAL(GPIO_BASE_ADDR + 0x5D) |= GPIO_SD1_WriteProtect;
			REG8_VAL(GPIO_BASE_ADDR + 0x43) |= GPIO_SD1_CD;
		}	
		

		/*  SD all pins output low  */
		if (SDDevInfo->CtrlID == 0) {
			REG8_VAL(GPIO_BASE_ADDR + 0xCD) &= ~SD0_PIN;
			REG8_VAL(GPIO_BASE_ADDR + 0xC3) &= ~GPIO_SD0_CD;
		}else if (SDDevInfo->CtrlID == 1) {
			REG8_VAL(GPIO_BASE_ADDR + 0xCF) &= ~GPIO_SD1_Data;
			REG8_VAL(GPIO_BASE_ADDR + 0xDD) &= ~GPIO_SD1_WriteProtect;
			REG8_VAL(GPIO_BASE_ADDR + 0xC3)	&= ~GPIO_SD1_CD;
		}


		/*  Config SD to GPO   */	
		if (SDDevInfo->CtrlID == 0) {
			REG8_VAL(GPIO_BASE_ADDR + 0x8D) |= SD0_PIN;
			REG8_VAL(GPIO_BASE_ADDR + 0x83) |= GPIO_SD0_CD;
		}else if (SDDevInfo->CtrlID == 1) {
			REG8_VAL(GPIO_BASE_ADDR + 0x8F) |= GPIO_SD1_Data;
			REG8_VAL(GPIO_BASE_ADDR + 0x9D) |= GPIO_SD1_WriteProtect;
			REG8_VAL(GPIO_BASE_ADDR + 0x83) |= GPIO_SD1_CD;
		}		
		/*Set SD1 CMD pin pull low because SD1CMD has no gpio pin*/
		if (SDDevInfo->CtrlID == 1) {
			REG8_VAL(GPIO_BASE_ADDR + 0x4DC) &= ~GPIO_SD1_Command;
			REG8_VAL(GPIO_BASE_ADDR + 0x49C) |= GPIO_SD1_Command;
		}		
		...
		if (SDDevInfo->CtrlID == 0) {
			...
			REG8_VAL(GPIO_BASE_ADDR + 0x48D) &= ~(GPIO_SD0_Clock | GPIO_SD0_Command);

			/*Set CD ,WP ,DATA pin pull up*/
			REG8_VAL(GPIO_BASE_ADDR + 0x4C3)|= GPIO_SD0_CD;
			REG8_VAL(GPIO_BASE_ADDR + 0x4CD) |= (GPIO_SD0_Data | GPIO_SD0_WriteProtect);

			/*Enable CD ,WP ,DATA  internal pull*/
			REG8_VAL(GPIO_BASE_ADDR + 0x483) |= GPIO_SD0_CD ;
			REG8_VAL(GPIO_BASE_ADDR + 0x48D) |= (GPIO_SD0_Data | GPIO_SD0_WriteProtect);
		}else if (SDDevInfo->CtrlID == 1) {
			/*Disable Clock & CMD Pull enable*/
			REG8_VAL(GPIO_BASE_ADDR + 0x49C) &= ~(GPIO_SD1_Clock | GPIO_SD1_Command);

			/*Set CD ,WP ,DATA pin pull up*/
			REG8_VAL(GPIO_BASE_ADDR + 0x4C3)|= GPIO_SD1_CD;
			REG8_VAL(GPIO_BASE_ADDR + 0x4CF) |= GPIO_SD1_Data;
			REG8_VAL(GPIO_BASE_ADDR + 0x4DD) |= GPIO_SD1_WriteProtect;
				
			/*Enable CD ,WP ,DATA  internal pull*/
			REG8_VAL(GPIO_BASE_ADDR + 0x483)|= GPIO_SD1_CD;
			REG8_VAL(GPIO_BASE_ADDR + 0x48F) |= GPIO_SD1_Data;
			REG8_VAL(GPIO_BASE_ADDR + 0x49D) |= GPIO_SD1_WriteProtect;
		}
