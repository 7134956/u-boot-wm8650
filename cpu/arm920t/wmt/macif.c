...
static void TD_RD_dump(void)
{
  int i;
	...
	for(i=0;i<sg_aAdapter->cbTD;i++){
		printf("TSR0:%02X TSR1:%02X OWNER:%02X\nBuffer length:%d Chain:%02X TCR:%02X\nBufferaddress:%08X\nNextTD:%08X\n"
			                ,sg_aAdapter->apTD[i]->m_td0TD0.byTSR0
			                ,sg_aAdapter->apTD[i]->m_td0TD0.byTSR1
			                ,sg_aAdapter->apTD[i]->m_td0TD0.f1Owner
			                ,sg_aAdapter->apTD[i]->m_td1TD1.f15TxBufLen
			                ,sg_aAdapter->apTD[i]->m_td1TD1.f1Chain
	...
	printf("RD dump-------------------------------------------------\n");
	for(i=0;i<sg_aAdapter->cbTD;i++){
		printf("RSR0:%02X RSR1:%02X receive length:%d OWNER:%02X\nBuffer length:%d\nBufferaddress:%08X\nNextRD:%08X\n"
			                ,sg_aAdapter->apRD[i]->m_rd0RD0.byRSR0
			                ,sg_aAdapter->apRD[i]->m_rd0RD0.byRSR1
			                ,sg_aAdapter->apRD[i]->m_rd0RD0.f15FrameLen
...
int mac_startio(bd_t* bd )
{
	...
	for (ii = 0; ii < CB_MAX_NET_DEVICE; ii++) {
		if ((&sg_aAdapter[ii])->wDevId == W_DEVICE_ID_3106A||(&sg_aAdapter[ii])->wDevId == W_DEVICE_ID_3053A) {
        	// Start driver:
	...
	#endif   
    g_bInit=TRUE; 
	//VNSvInPortB(pAdapter->dwIoBase+ii, &check)
	// printf("macaddr = %x\n",check);     
	return 0;		
...
int mac_receive(void)
{
	...
	while(TRUE){
		...
		//else ,error handling
			...
			printf("receive error status:%02X %02X%\n",pRD->m_rd0RD0.byRSR1,pRD->m_rd0RD0.byRSR0);
